...
# )
# ... (rest of the imports and initial code remains the same)

parser.add_argument(
    '--spe_character_coverage',
    type=float,
    default=0.9995,  # Adjusted for Persian
    help="Character coverage percentage for SentencePiece tokenization. For languages "
    "with large vocabulary, should be close to 0.9995, otherwise kept as 1.0",
)
parser.add_argument('--no_lower_case', dest='lower_case', action='store_false', default=True)  # Persian doesn't have uppercase/lowercase
parser.add_argument(
    '--spe_no_split_by_unicode_script',
    dest='spe_split_by_unicode_script',
    action='store_false',
    default=True,  # Disable Unicode script splitting for Persian
    help="Don't use Unicode script to split sentence pieces.",
)
parser.add_argument(
    '--spe_user_defined_symbols',
    default=[
        "ا", "ب", "پ", "ت", "ث", "ج", "چ", "ح", "خ", "د", "ذ", "ر", "ز", "ژ", "س", "ش", "ص", "ض", "ط", "ظ", "ع", "غ", "ف", "ق", "ک", "گ", "ل", "م", "ن", "و", "ه", "ی",  # Persian letters
        "،", "؛", "؟", "«", "»", "ـ", " ", ".", ")", "(", "1", "2", "3", "4", "5", "6", "7", "8", "9", "", "۱", "۲", "۳", "۴", "۵", "۶", "۷", "۸", "۹", "۰", "-"   # Persian punctuation marks
    ],
    type=str,
    nargs='+',
